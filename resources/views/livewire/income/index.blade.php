    ...
    <x-table>
        ...
        <x-table.tbody>
            @forelse ($incomes as $income)
                <x-table.tr wire:loading.class.delay="opacity-50" wire:key="row-{{ $income->id }}">
                    <x-table.td class="pr-0">
                        <input wire:model="selected" type="checkbox" value="{{ $income->id }}" />
                    </x-table.td>
                    <x-table.td>
                        <button type="button" wire:click="showModal({{ $income->id }})">
                            {{ $income->reference }}
                        </button>
                    </x-table.td>
                    <x-table.td>
                    ...
                    <x-table.td>
                        <div class="flex justify-start space-x-2">
                            ...
                            <x-button primary wire:click="$emit('editModal',{{ $income->id }})" type="button"
                                wire:loading.attr="disabled">
                ...
                <x-table.tr>
                    <x-table.td colspan="7">
                        <div class="flex justify-center items-center space-x-2">
                            ...
                            <span
                                class="font-medium py-8 text-gray-400 text-xl">{{ __('No incomes found...') }}</span>
                        </div>
                    </x-table.td>
                </x-table.tr>
            @endforelse
            ...
            <div class="w-full pb-5">
                <div class="flex flex-wrap">
                    <div class="lg:w-1/2 sm:w-full px-2">
                        <x-label for="category_id" :value="__('Income Category')" />
                        {{ $this->income?->category?->name }}
                    </div>
                    <div class="lg:w-1/2 sm:w-full px-2">
                        <x-label for="category_id" :value="__('Warehouse')" />
                        {{ $this->income?->warehouse?->name }}
                    </div>
                    <div class="lg:w-1/2 sm:w-full px-2">
    ...
    @push('scripts')
        <script>
            document.addEventListener('livewire:load', function() {
                window.livewire.on('deleteModal', incomeId => {
                    Swal.fire({
                        title: '¿Estás seguro?',
                        ...
                        if (result.isConfirmed) {
                            window.livewire.emit('delete', incomeId)
                        }
                    })
                })
            })
